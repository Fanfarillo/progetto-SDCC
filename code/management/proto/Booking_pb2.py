# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: proto/Booking.proto
"""Generated protocol buffer code."""
from google.protobuf import descriptor as _descriptor
from google.protobuf import descriptor_pool as _descriptor_pool
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor_pool.Default().AddSerializedFile(b'\n\x13proto/Booking.proto\x12\x05proto\"%\n\x12\x41vailableSeatReply\x12\x0f\n\x07idPosto\x18\x01 \x01(\t\"&\n\x14\x41vailableSeatRequest\x12\x0e\n\x06idVolo\x18\x01 \x01(\t\"x\n\x14getAllFlightsRequest\x12\x0e\n\x06giorno\x18\x01 \x01(\x05\x12\x0c\n\x04mese\x18\x02 \x01(\x05\x12\x0c\n\x04\x61nno\x18\x03 \x01(\x05\x12\x18\n\x10\x61\x65roporto_arrivo\x18\x04 \x01(\t\x12\x1a\n\x12\x61\x65roporto_partenza\x18\x05 \x01(\t\"!\n\x10postiDisponibili\x12\r\n\x05posti\x18\x01 \x03(\t\"\xc1\x01\n\x12getAllFlightsReply\x12\n\n\x02id\x18\x01 \x01(\t\x12\x11\n\tcompagnia\x18\x02 \x01(\t\x12\x0e\n\x06\x61rrivo\x18\x03 \x01(\t\x12\x10\n\x08partenza\x18\x04 \x01(\t\x12\x0c\n\x04\x64\x61ta\x18\x05 \x01(\t\x12\x0e\n\x06orario\x18\x06 \x01(\t\x12\x12\n\nprezzoBase\x18\x07 \x01(\t\x12&\n\x05posti\x18\x08 \x01(\x0b\x32\x17.proto.postiDisponibili\x12\x10\n\x08numPosti\x18\t \x01(\x05\"\x17\n\tIdMessage\x12\n\n\x02id\x18\x01 \x01(\t\"\x1a\n\nIdResponse\x12\x0c\n\x04isOk\x18\x01 \x01(\x08\"\xb3\x01\n\nNewFlight2\x12\n\n\x02id\x18\x01 \x01(\t\x12\x0c\n\x04\x64\x61te\x18\x02 \x01(\t\x12\x18\n\x10\x64\x65partureAirport\x18\x03 \x01(\t\x12\x16\n\x0e\x61rrivalAirport\x18\x04 \x01(\t\x12\x15\n\rdepartureTime\x18\x05 \x01(\t\x12\x13\n\x0b\x61rrivalTime\x18\x06 \x01(\t\x12\x0f\n\x07\x61irline\x18\x07 \x01(\t\x12\r\n\x05price\x18\x08 \x01(\t\x12\r\n\x05seats\x18\t \x01(\x05\" \n\x10RegisterResponse\x12\x0c\n\x04isOk\x18\x01 \x01(\x08\"E\n\x0eUpdatedFlight2\x12\x10\n\x08\x66lightId\x18\x01 \x01(\t\x12\x10\n\x08newPrice\x18\x02 \x01(\t\x12\x0f\n\x07\x61irline\x18\x03 \x01(\t\"\x1e\n\x0eUpdateResponse\x12\x0c\n\x04isOk\x18\x01 \x01(\x08\"\"\n\x0f\x41irportsRequest\x12\x0f\n\x07isDummy\x18\x01 \x01(\x08\"F\n\x10\x41irportsResponse\x12\x19\n\x11\x64\x65partureAirports\x18\x01 \x03(\t\x12\x17\n\x0f\x61rrivalAirports\x18\x02 \x03(\t\",\n\x14GetLogFileRequestBoo\x12\x14\n\x0cnumRichiesta\x18\x01 \x01(\x05\";\n\x12GetLogFileReplyBoo\x12\x12\n\nchunk_file\x18\x01 \x01(\x0c\x12\x11\n\tnum_chunk\x18\x02 \x01(\x05\x32\xf1\x03\n\x0e\x42ookingService\x12I\n\rgetAllFlights\x12\x1b.proto.getAllFlightsRequest\x1a\x19.proto.getAllFlightsReply0\x01\x12-\n\x06SendId\x12\x10.proto.IdMessage\x1a\x11.proto.IdResponse\x12<\n\x0eRegisterFlight\x12\x11.proto.NewFlight2\x1a\x17.proto.RegisterResponse\x12\x41\n\x11UpdateFlightPrice\x12\x15.proto.UpdatedFlight2\x1a\x15.proto.UpdateResponse\x12Y\n\x1dgetAllAvailableSeatsForFlight\x12\x1b.proto.AvailableSeatRequest\x1a\x19.proto.AvailableSeatReply0\x01\x12>\n\x0bGetAirports\x12\x16.proto.AirportsRequest\x1a\x17.proto.AirportsResponse\x12I\n\rgetLogFileBoo\x12\x1b.proto.GetLogFileRequestBoo\x1a\x19.proto.GetLogFileReplyBoo0\x01\x62\x06proto3')



_AVAILABLESEATREPLY = DESCRIPTOR.message_types_by_name['AvailableSeatReply']
_AVAILABLESEATREQUEST = DESCRIPTOR.message_types_by_name['AvailableSeatRequest']
_GETALLFLIGHTSREQUEST = DESCRIPTOR.message_types_by_name['getAllFlightsRequest']
_POSTIDISPONIBILI = DESCRIPTOR.message_types_by_name['postiDisponibili']
_GETALLFLIGHTSREPLY = DESCRIPTOR.message_types_by_name['getAllFlightsReply']
_IDMESSAGE = DESCRIPTOR.message_types_by_name['IdMessage']
_IDRESPONSE = DESCRIPTOR.message_types_by_name['IdResponse']
_NEWFLIGHT2 = DESCRIPTOR.message_types_by_name['NewFlight2']
_REGISTERRESPONSE = DESCRIPTOR.message_types_by_name['RegisterResponse']
_UPDATEDFLIGHT2 = DESCRIPTOR.message_types_by_name['UpdatedFlight2']
_UPDATERESPONSE = DESCRIPTOR.message_types_by_name['UpdateResponse']
_AIRPORTSREQUEST = DESCRIPTOR.message_types_by_name['AirportsRequest']
_AIRPORTSRESPONSE = DESCRIPTOR.message_types_by_name['AirportsResponse']
_GETLOGFILEREQUESTBOO = DESCRIPTOR.message_types_by_name['GetLogFileRequestBoo']
_GETLOGFILEREPLYBOO = DESCRIPTOR.message_types_by_name['GetLogFileReplyBoo']
AvailableSeatReply = _reflection.GeneratedProtocolMessageType('AvailableSeatReply', (_message.Message,), {
  'DESCRIPTOR' : _AVAILABLESEATREPLY,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.AvailableSeatReply)
  })
_sym_db.RegisterMessage(AvailableSeatReply)

AvailableSeatRequest = _reflection.GeneratedProtocolMessageType('AvailableSeatRequest', (_message.Message,), {
  'DESCRIPTOR' : _AVAILABLESEATREQUEST,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.AvailableSeatRequest)
  })
_sym_db.RegisterMessage(AvailableSeatRequest)

getAllFlightsRequest = _reflection.GeneratedProtocolMessageType('getAllFlightsRequest', (_message.Message,), {
  'DESCRIPTOR' : _GETALLFLIGHTSREQUEST,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.getAllFlightsRequest)
  })
_sym_db.RegisterMessage(getAllFlightsRequest)

postiDisponibili = _reflection.GeneratedProtocolMessageType('postiDisponibili', (_message.Message,), {
  'DESCRIPTOR' : _POSTIDISPONIBILI,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.postiDisponibili)
  })
_sym_db.RegisterMessage(postiDisponibili)

getAllFlightsReply = _reflection.GeneratedProtocolMessageType('getAllFlightsReply', (_message.Message,), {
  'DESCRIPTOR' : _GETALLFLIGHTSREPLY,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.getAllFlightsReply)
  })
_sym_db.RegisterMessage(getAllFlightsReply)

IdMessage = _reflection.GeneratedProtocolMessageType('IdMessage', (_message.Message,), {
  'DESCRIPTOR' : _IDMESSAGE,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.IdMessage)
  })
_sym_db.RegisterMessage(IdMessage)

IdResponse = _reflection.GeneratedProtocolMessageType('IdResponse', (_message.Message,), {
  'DESCRIPTOR' : _IDRESPONSE,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.IdResponse)
  })
_sym_db.RegisterMessage(IdResponse)

NewFlight2 = _reflection.GeneratedProtocolMessageType('NewFlight2', (_message.Message,), {
  'DESCRIPTOR' : _NEWFLIGHT2,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.NewFlight2)
  })
_sym_db.RegisterMessage(NewFlight2)

RegisterResponse = _reflection.GeneratedProtocolMessageType('RegisterResponse', (_message.Message,), {
  'DESCRIPTOR' : _REGISTERRESPONSE,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.RegisterResponse)
  })
_sym_db.RegisterMessage(RegisterResponse)

UpdatedFlight2 = _reflection.GeneratedProtocolMessageType('UpdatedFlight2', (_message.Message,), {
  'DESCRIPTOR' : _UPDATEDFLIGHT2,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.UpdatedFlight2)
  })
_sym_db.RegisterMessage(UpdatedFlight2)

UpdateResponse = _reflection.GeneratedProtocolMessageType('UpdateResponse', (_message.Message,), {
  'DESCRIPTOR' : _UPDATERESPONSE,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.UpdateResponse)
  })
_sym_db.RegisterMessage(UpdateResponse)

AirportsRequest = _reflection.GeneratedProtocolMessageType('AirportsRequest', (_message.Message,), {
  'DESCRIPTOR' : _AIRPORTSREQUEST,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.AirportsRequest)
  })
_sym_db.RegisterMessage(AirportsRequest)

AirportsResponse = _reflection.GeneratedProtocolMessageType('AirportsResponse', (_message.Message,), {
  'DESCRIPTOR' : _AIRPORTSRESPONSE,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.AirportsResponse)
  })
_sym_db.RegisterMessage(AirportsResponse)

GetLogFileRequestBoo = _reflection.GeneratedProtocolMessageType('GetLogFileRequestBoo', (_message.Message,), {
  'DESCRIPTOR' : _GETLOGFILEREQUESTBOO,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.GetLogFileRequestBoo)
  })
_sym_db.RegisterMessage(GetLogFileRequestBoo)

GetLogFileReplyBoo = _reflection.GeneratedProtocolMessageType('GetLogFileReplyBoo', (_message.Message,), {
  'DESCRIPTOR' : _GETLOGFILEREPLYBOO,
  '__module__' : 'proto.Booking_pb2'
  # @@protoc_insertion_point(class_scope:proto.GetLogFileReplyBoo)
  })
_sym_db.RegisterMessage(GetLogFileReplyBoo)

_BOOKINGSERVICE = DESCRIPTOR.services_by_name['BookingService']
if _descriptor._USE_C_DESCRIPTORS == False:

  DESCRIPTOR._options = None
  _AVAILABLESEATREPLY._serialized_start=30
  _AVAILABLESEATREPLY._serialized_end=67
  _AVAILABLESEATREQUEST._serialized_start=69
  _AVAILABLESEATREQUEST._serialized_end=107
  _GETALLFLIGHTSREQUEST._serialized_start=109
  _GETALLFLIGHTSREQUEST._serialized_end=229
  _POSTIDISPONIBILI._serialized_start=231
  _POSTIDISPONIBILI._serialized_end=264
  _GETALLFLIGHTSREPLY._serialized_start=267
  _GETALLFLIGHTSREPLY._serialized_end=460
  _IDMESSAGE._serialized_start=462
  _IDMESSAGE._serialized_end=485
  _IDRESPONSE._serialized_start=487
  _IDRESPONSE._serialized_end=513
  _NEWFLIGHT2._serialized_start=516
  _NEWFLIGHT2._serialized_end=695
  _REGISTERRESPONSE._serialized_start=697
  _REGISTERRESPONSE._serialized_end=729
  _UPDATEDFLIGHT2._serialized_start=731
  _UPDATEDFLIGHT2._serialized_end=800
  _UPDATERESPONSE._serialized_start=802
  _UPDATERESPONSE._serialized_end=832
  _AIRPORTSREQUEST._serialized_start=834
  _AIRPORTSREQUEST._serialized_end=868
  _AIRPORTSRESPONSE._serialized_start=870
  _AIRPORTSRESPONSE._serialized_end=940
  _GETLOGFILEREQUESTBOO._serialized_start=942
  _GETLOGFILEREQUESTBOO._serialized_end=986
  _GETLOGFILEREPLYBOO._serialized_start=988
  _GETLOGFILEREPLYBOO._serialized_end=1047
  _BOOKINGSERVICE._serialized_start=1050
  _BOOKINGSERVICE._serialized_end=1547
# @@protoc_insertion_point(module_scope)
